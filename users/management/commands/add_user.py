...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        if not User.objects.filter(email="user1@mail.ru").exists():
            user = User.objects.create(email="user1@mail.ru")
        ...
        if not User.objects.filter(email="user2@mail.ru").exists():
